...
export class DB_NAME {
  ...
  public static readonly CORE_USERS = "coreUsers";
  public static readonly LOAN_USERS = "loanUsers";
}
...
export class DBService {
  ...
  getDataFrom(tbl:string): Observable<any> {
    return this.afs.collection(tbl).snapshotChanges().map(changes => {
  ...
  updateData(tbl:string, data:any): Promise<void> {
    if (data._id){
      return this.afs.doc(tbl+"/"+data._id).update(data).catch(error => {
        console.log(error);
      });  
    } else {
      const pushkey = this.afs.createId();
      const model = {'_id': pushkey, ...data };
      return this.afs.doc(tbl+"/"+pushkey).set(model).catch(error => {
        console.log(error);
      });
    }
  }
