...
@Injectable()
export class OneVideoService implements Resolve<any> {
  ...
  public widgets = {
    ...
    getVideoList(): Promise<any> {
        return new Promise((resolve, reject) => {
          this.afs.collection('VideoONE').snapshotChanges().map(changes => {
            return changes.map(c => ({id: c.payload.doc.id, ...c.payload.doc.data()}));
          }).subscribe(data => {
            this.videoList = data;
            this.onVideoListChanged.next(data);
            resolve(data);
          }, reject);
        });
    }
    ...
    updateVideo(video) {
        const id = video.id;
        return this.afs.doc(`VideoONE/${id}`).update(video);
    }
}
